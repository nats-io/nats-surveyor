...
func TestSurveyor_AccountJetStreamJszLeaderOnly(t *testing.T) {
	...
	expectedConsumers := 120
	if totalConsumers != expectedConsumers {
		t.Errorf("Expected %v, got %v", expectedConsumers, totalConsumers)
	}
	expectedStreams := 60
	if totalStreams != expectedStreams {
		t.Errorf("Expected %v, got %v", expectedStreams, totalStreams)
	}
}
